import 'package:flutter/material.dart';

//constants color range for theme
const Color kDodgerBlue = Color.fromRGBO(29, 161, 242, 1);
const Color kButtonColor = Color(0xFF207398);
const Color kbuttonTextColor = Colors.white;
Color bgOverlayColor = Color(0xFF125E80).withOpacity(0.80);

const Color kWhiteLilac = Color.fromRGBO(248, 250, 252, 1);
const Color kBlackPearl = Color.fromRGBO(30, 31, 43, 1);
const Color kBrinkPink = Color.fromRGBO(255, 97, 136, 1);
const Color kJuneBud = Color.fromRGBO(186, 215, 97, 1);

const Color kBlack = Colors.black;
const Color kGrey = Colors.grey;
const Color kWhite = Color(0xFFFFFFFF);
const Color kNevada = Color.fromRGBO(105, 109, 119, 1);
const Color darkGrey = Color(0xFF858585);
const Color textFormFieldColor = Color(0xFFB082D6);

const Color kEbonyClay = Color.fromRGBO(40, 42, 58, 1);
const Color primaryColor = Color(0xFF953AE4);
const Color primaryLightColor = Color(0xFFF1EAF8);
const Color primaryDarkColor = Color(0xFF531C81);

const Color grey1 = Color(0xFF717171);
const Color grey3 = Color(0xFFD2D0D0);
const Color grey2 = Color(0xFF858585);
const Color boxColor = Color(0xFFF1F1F1);

const Color bottomnavLightbackgroundColor = Color(0xFFEEEEEE);
const Color bottomnavdarkbackgroundColor = Color(0xFF262625);
const Color darkBackgroundColor = Color(0xFFE8EDF4);

const Color redColor = Color(0xFFFF0000);
const Color blueColor = Color(0xFF3385FF);
LinearGradient gradientColor = const LinearGradient(
  colors: [
    Color(0xFFB3803B),
    Color(0xFFF4E486),
    Color(0xFFF2E084),
    Color(0xFF8F5E26)
  ],
);
LinearGradient buttonGradient = const LinearGradient(
  // tileMode: TileMode.repeated,
  end: Alignment.bottomCenter,
  begin: Alignment.topCenter,
  colors: [
    Color(0xFFB3803B),
    Color(0xFFF4E486),
    Color(0xFFF2E084),
    Color(0xFF8F5E26)
  ],
);
LinearGradient gradientColorLight = const LinearGradient(
  colors: [
    Color(0xFFB3803B),
    Color(0xFFC69A58),
    Color(0xFFD7A760),
    Color(0xFF8F5E26)
  ],
);
